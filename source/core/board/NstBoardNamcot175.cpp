	...
	namespace Core
	{
		namespace Boards
		{
			namespace Namcot
			{
				...
				N175::Sound::Sound(Apu& a,bool connect)
				: Channel(a)
				{
					Reset();
				...
				void N175::Sound::BaseChannel::Reset()
				{
					enabled = false;
					active = false;
					timer = 0;
				...
				byte* N175::Sound::GetExRam()
				{
					return exRam;
				}

				const byte* N175::Sound::GetExRam() const
				{
					return exRam;
				}

				void N175::Sound::SaveState(State::Saver& state,const dword baseChunk) const
				{
				...
				void N175::Sound::LoadState(State::Loader& state)
				{
				...
				inline bool N175::Sound::BaseChannel::CanOutput() const
				{
					return volume && frequency && enabled;
				}

				inline void N175::Sound::BaseChannel::Validate()
				{
					active = CanOutput();
				}

				inline void N175::Sound::BaseChannel::SetFrequency(const uint f)
				{
					frequency = f;
				}

				inline void N175::Sound::BaseChannel::SetWaveLength(const uint data)
				{
				...
				inline void N175::Sound::BaseChannel::SetWaveOffset(const uint data)
				{
					waveOffset = data;
				}

				inline void N175::Sound::BaseChannel::SetVolume(const uint data)
				{
					volume = (data & REG_VOLUME) * VOLUME;
				}

				inline dword N175::Sound::BaseChannel::GetSample
				(
					const Cycle rate,
					const Cycle factor,
				...
				N175::Sound::Sample N175::Sound::GetSample()
				{
					if (output)
					{
						dword sample = 0;
				...
				bool N175::Sound::UpdateSettings()
				{
					//uint volume = GetVolume(EXT_N175) * 68U / DEFAULT_VOLUME;
				...
				inline void N175::Sound::SetChannelState(uint data)
				{
					data = (data >> 4 & 0x7) + 1;
				...
				inline dword N175::Sound::FetchFrequency(uint address) const
				{
					address &= 0x78;
				...
				inline void N175::Sound::WriteWave(const uint data)
				{
				...
				void N175::Sound::WriteData(const uint data)
				{
					Update();
				...
				void N175::Sound::WriteAddress(const uint data)
				{
